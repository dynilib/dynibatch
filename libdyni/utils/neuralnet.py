...
def build_cnn_audio(batch_size, num_time_bins, num_classes):
    ...
    network = lasagne.layers.Conv1DLayer(network, cl1_num_filters, cl1_filter_size,
                                         ...
                                         # # TODO: make sure it's scaled according to input_size
                                         b=lasagne.init.Constant(0.),
                                         nonlinearity=lasagne.nonlinearities.rectify,
                                         # TODO apply that after pooling to save some useless calculation
...
def train_cnn(sc_gen_train,
    ...
    train_loss = train_loss.mean() + reg_coef * lasagne.regularization.regularize_network_params(network,
    ...
    valid_prediction = lasagne.layers.get_output(network,
                                                 deterministic=True)  # deterministic=True disables the dropout layers
